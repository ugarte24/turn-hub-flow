-- PostgREST no puede elegir entre dos firmas de generate_ticket
-- (4 args vs 5 args con _created_by). Dejamos solo la versión actual.

DROP FUNCTION IF EXISTS public.generate_ticket(TEXT, UUID, UUID);
DROP FUNCTION IF EXISTS public.generate_ticket(TEXT, UUID, UUID, UUID);
...
GRANT EXECUTE ON FUNCTION public.generate_ticket(TEXT, UUID, UUID, UUID, UUID) TO anon, authenticated, service_role;
